...
#define SERVER_PORT 42240  // 请替换为您的端口号配置
#define MAXBUFLEN 1024
#define ROOM_STATUS_FILE "double.txt"
...
void handleRequests(int sockfd, RoomStatus *roomStatusArray, int roomStatusCount) {
    ...
    while (1) {
        ...
        if ((numbytes = recvfrom(sockfd, buf, MAXBUFLEN - 1, 0,
            ...
            perror("serverD: recvfrom");
            exit(1);
        }

        // 确保字符串以null终止
        buf[numbytes] = '\0';
        printf("serverD: Received request: %s\n", buf);
...
int main(void) {
    ...
    for (int i = 0; i < roomStatusCount; i++) {
        ...
        if (numbytes == -1) {
            perror("serverD: sendto main server");
            exit(1);
        }
        break;
    }
